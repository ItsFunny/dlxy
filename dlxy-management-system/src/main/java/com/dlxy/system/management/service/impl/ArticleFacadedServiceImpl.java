...
public class ArticleFacadedServiceImpl implements IArticleFacadedService
{
	...
	public void updateArticlesInBatch(Long userId, Long[] articleIds, int status)
	{
		...
		userRecordDTO.setRecordDetail("delete:article:" + sb.toString());
		userRecordService.addRecord(userRecordDTO);
	}

	@Override
	public PageDTO<Collection<ArticleDTO>> serarchByParams(int start, int end, Map<String, Object> params)
			throws SQLException
	{
		Long totalCount = articleCountService.countArticlesByDetailParam(params);
		if (totalCount <= 0)
